...
select job from EMP disnict job

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 커미션_PCT(comm_pct), 급여에 커미션_PCT를 곱한 값을 조회.
select emp_id, emp_name, salarym comm_pct, salary * comm_pct from EMP
...
select emp_id, emp_name from EMP where emp_name like '%'
...
select * from EMP where emp_name = 'Sales' and job = 'SA_MAN'
...
select emp_id, emp_name, job, salary from EMP order by job desc, salary asc

-- 부서명을 부서명(dept_name)의 오름차순으로 정렬해 조회하시오.
select dept_name from EMP order by dept_name asc

-- 급여(salary)가 $5,000을 넘는 직원의 ID(emp_id), 이름(emp_name), 급여(salary)를 급여가 높은 순서부터 조회
select emp_id, emp_name, salary from EMP where salary > 50000 order by salary desc

...
select e.comm_pct, e.emp_id, e.emp_name, e.salary, d.dept_name, d.loc from emp e join dept d on e.dept_id = d.dept_id where e.comm_pct is not null order by e.emp_id desc;

-- 직원의 ID(emp.emp_id)가 100인 직원의 직원_ID(emp.emp_id), 이름(emp.emp_name), 입사년도(emp.hire_date), 소속부서이름(dept.dept_name)을 조회.
select e.emp_id, e.emp_name, e.hire_date, d.dept_name from emp e join dept d on e.dept_id = d.dept_id where e.emp_id = 100;

-- 직원_ID(emp.emp_id), 이름(emp.emp_name), 급여(emp.salary), 담당업무명(job.job_title), 소속부서이름(dept.dept_name)을 조회
select e.emp_id, e.emp_name, e.salary, j.job_title, d.dept_name from emp e join job j  on e.job_id = j.job_id join dept d on e.dept_id = d.dept_id order by 1;


--  직원 ID 가 200 인 직원의 직원_ID(emp.emp_id), 이름(emp.emp_name), 급여(emp.salary), 담당업무명(job.job_title), 소속부서이름(dept.dept_name)을 조회              
select e.emp_id, e.emp_name, e.salary, j.job_title, d.dept_name from emp e join job j on e.job_id = j.job_id join dept d on e.dept_id = d.dept_id where emp.emp_id = 200;
...
select e.emp_id, e.emp_name, e.salary, s.grade as '급여등급' from emp e join salary_grade s  on e.salary between s.low_sal and s.high_sal
...
select d.dept_id, d.dept_name, d.loc, e.emp_id, e.emp_name, e.job_jd from dept d join emp e on d.dept_id = e.dept_id where d.loc = 'New York';


-- 부서별 급여(salary)의 평균을 조회. 부서이름(dept.dept_name)과 급여평균을 출력. 급여 평균이 높은 순서로 정렬. 
select d.dept_id, d.dept_name, avg(salary) as "평균급여" from dept d join emp e on d.dept_id = e.dept_id
...
select e.emp_id, e.emp_name, j.job_title, e.salary, concat(s.grade, '등급') as 'grade', d.dept_name from emp e join job j on e.job_id = j.job_id join salary_grade s on e.salary between s.low_sal and s.high_sal join dept d on e.dept_id = d.dept_id order by 1;
...
select e.emp_id, e.emp_name, m.emp_name as 'manager name' from emp e join emp m on e.mgr_id =m.emp_id
...
select e.emp_id, e.emp_name, e.salary, d.dept_name, d.loc from emp e left join dept d on e.dept_id = d.dept_id order by 4;
...
select e.emp_id, e.emp_name, e.dept_id, d.loc from emp e left join dept d on e.dept_id = d.dept_id and d.dept = 80
        
--  직원_id(emp.emp_id)가 100, 110, 120, 130, 140인 직원의 ID(emp.emp_id),이름(emp.emp_name), 업무명(job.job_title) 을 조회. 업무명이 없을 경우 '미배정' 으로 조회
select e.emp_id, e.emp_name, ifnull(j.job_title, '미배정') as 'job_title' from emp e left join job j on e.job_id = j.job_id where e.emp_id in (100, 110, 120, 130, 140)
...
select d.dept_id, d.dept_name, count(e.emp_id) as "직원수" from dept d left join emp e on d.dept_id = e.dept_id group by d.dept_id, d.dept_name order by 3 desc;
...
select e.emp_id, e.emp_name, m.emp_name as "상사이름", date_format(e.hire_date, '%Y%m%d') as "hire_date" from emp e left join emp m on e.mgr_id = m.emp_id where e.dept_id = 90
...
select e.emp_id, e.emp_name, j.job_title, e.salary, e.hire_date, m.emp_name as "상사이름", m.hire_date as '상사입사일', d.dept_name, d.loc from emp e left join job j on e.job_id = j.job_id left join emp m on e.mgr_id = m.emp_id left join dept d on e.dept_id = d.dept_id

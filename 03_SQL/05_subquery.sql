...
select * from emp where (job_id, dept_id) = (select job_id, dept_id from emp where emp_id = 115)
...
select emp_id, emp_name, job_id from emp where (job_id, mgr_id) = (select job_id, mgr_id from emp where emp_id=150);


-- 직원들 중 급여(emp.salary)가 전체 직원의 평균 급여보다 적은 직원들의 id(emp.emp_id), 이름(emp.emp_name), 급여(emp.salary)를 조회. 
select emp_id, emp_name, salary from emp where salary < (select avg(salary) from emp order by 3 desc;


-- 부서직원들의 평균이 전체 직원의 평균(emp.salary) 이상인 부서의 이름(dept.dept_name), 평균 급여(emp.salary) 조회.
-- 평균급여는 소숫점 2자리까지 나오고 통화표시($)와 단위 구분자 출력
select d.dept_id, d.dept_name, concat('$',round(avg(e.salary),2)) as "평균급여" from dept d join emp e on d.dept_id = d.dept_id group by d.dept_id, d.dept_name having avg(salary) >= (select avg(salary) from emp) order by 3;

select dept_id, dept_name, concat('$', round(평균급여,2)) as "평균급여" from (select d.dept_id, d.dept_name, avg(e.salary) as "평균급여" from dept d join emp e on d.dept_id = d.dept_id group by d.dept_id, d.dept_name having avg(salary) >= (select avg(salary) from emp) order by 3) as t;


--  급여(emp.salary)가장 많이 받는 직원이 속한 부서의 이름(dept.dept_name), 위치(dept.loc)를 조회.
select dept_name, loc from dept where dept_id = (select dept_id from emp where salary = (select max(salary) from emp));



-- Sales 부서(dept.dept_name) 의 평균 급여(emp.salary)보다 급여가 많은 직원들의 모든 정보를 조회.
select * from emp where salary > (select avg(salary) from dept d join emp e on d.dept_id = e.dept_id where d.dept_name = 'Sales')
order by salary asc;


-- 전체 직원들 중 담당 업무 ID(emp.job_id) 가 'ST_CLERK'인 직원들의 평균 급여보다 적은 급여를 받는 직원들의 모든 정보를 조회. 
-- 단 업무 ID가 'ST_CLERK'이 아닌 직원들만 조회. 
select * from emp where salary > (select avg(salary) from emp where job_id = 'ST_CLERK') AND (job_id <> 'ST_CLERK' or job_id is null)
...
select emp_id, emp_name, salary from emp where salary >(select max(salary) from emp where job_id = 'IT_PROG') order by 3;
...
select empi_id, emp_name, job_id, year(hire_date) as '입사연도', salary from emp where mgr_id in (select emp_id from emp where emp_name = 'Alexander');

--  부서 위치(dept.loc) 가 'New York'인 부서에 소속된 직원의 ID(emp.emp_id), 이름(emp.emp_name), 부서_id(emp.dept_id) 를 sub query를 이용해 조회.
select emp_id, emp_name, dept_id from emp where dept_id in (select dept_id from dept where loc='New York');
...
select * from emp where salary > any(select salary from emp where emp_id in (101, 102, 103));
...
-- 중복된 데이터 조회 
select emp_name from (select emp_name, count(*) as 'cnt' from emp group by emp_name) as t where cnt >= 2;
...
select d.dept_id, d.dept_nme, d.loc from dept d where exists (select * from emp e where e.dept_id = d.dept_id);

-- subquery에서 main query의 table을 사용하도록 정의하면 상관 쿼리.


-- 직원이 한명도 없는 부서의 부서ID(dept.dept_id)와 이름(dept.dept_name), 위치(dept.loc)를 조회
select d.dept_id, d.dept_nme, d.loc from dept d where not exists (select * from emp e where e.dept_id = d.dept_id);
